...
import {
	constructPositionedMatchesBasic,
	initializeBracket,
} from "../../utils/bracketDistribution";
...
const BracketMatch = ({ matchId, startPosition, endPosition, stageNumber }: BracketMatchProps) => {
		...
		<>
			<>
				...
				<StyledStageItem
					...
					$isSecondPlayer={true}
				>
					{match.firstPlayer && match.secondPlayer && !match.isFinished && (
						<StyledMatchTrigger
							onClick={() => {
								if (!match.isStarted) dispatch(startMatch(matchId)); else dispatch(finishMatch(matchId));
							}}
...
const Stage = ({ matchIds, stageNumber }: StageProps) => {
