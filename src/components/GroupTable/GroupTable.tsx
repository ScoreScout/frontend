  ...
  return (
    <TableContainer numPlayers={numOfPlayers}>
      <HeadRow numPlayers={numOfPlayers}>
        <Cell rowIndex={0} cellIndex={0} numPlayers={numOfPlayers}>
          First group
        </Cell>
        {[...Array(numOfPlayers)].map((_, index) => (
          <Cell key={index} rowIndex={0} cellIndex={index + 1} numPlayers={numOfPlayers}>
            {index + 1}
          </Cell>
        ))}
        <Cell rowIndex={0} cellIndex={numOfPlayers + 1} numPlayers={numOfPlayers}>
          Points
        </Cell>
